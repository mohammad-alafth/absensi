        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-5">
                ...
                <p class="text-sm text-gray-500 mt-1 mb-5">
                    Daftar pengajuan izin yang sudah disetujui PJ dan menunggu verifikasi HRD
                </p>
                ...
                <div x-data="{ showDetail: false, rejectModal: false, approveModal: false }"
                    ...
                    <div x-show="showDetail"
                        ...
                        <div @click.away="showDetail = false"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                    <p class="text-gray-500">Tanggal Izin</p>
                                    <p class="font-semibold">{{ \Carbon\Carbon::parse($item->tanggal)->translatedFormat('d F Y') }}@if($item->tanggal_selesai && $item->tanggal_selesai != $item->tanggal) s/d {{ \Carbon\Carbon::parse($item->tanggal_selesai)->translatedFormat('d F Y') }}@endif</p>
                                </div>
                                @if($item->jam_mulai)
                                <div>
                                    <p class="text-gray-500">Jam Mulai</p>
                                    <p class="font-semibold">{{ $item->jam_mulai }}</p>
                                </div>
                                @endif
                                @if($item->jam_selesai)
                                <div>
                                    <p class="text-gray-500">Jam Selesai</p>
                                    <p class="font-semibold">{{ $item->jam_selesai }}</p>
                                </div>
                                @endif
                                <div>
                        ...
                        <div class="flex flex-col lg:flex-row justify-between gap-4">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">Jenis: <span class="font-semibold text-indigo-600">{{ $item->jenis }}</span></p>
                                    </div>
                                </div>
                                ...
                                <div class="mt-4 space-y-2">
                                    ...
                                    <p class="text-sm">
                                        <span class="font-semibold">Waktu / Jam:</span>
                                        {{ $item->jam_mulai ?? '00:00' }} s/d {{ $item->jam_selesai ?? 'Selesai' }}
                                    </p>
                                    @endif
                                    <p class="text-sm">
                                        <span class="font-semibold">Disetujui PJ:</span>
                                        {{ $item->pjApprover->name ?? '-' }}
                                    </p>
                                </div>
                            ...
                            <div class="flex flex-col gap-3 lg:w-[180px]">
                                ...
                                <button @click="showDetail = true" type="button"
                                    class="bg-indigo-500 hover:bg-indigo-600 text-white px-5 py-2 rounded-xl font-medium transition text-sm">
                                    Detail
                                </button>

                                @if(isset($item->pdf_file) && $item->pdf_file)
                                <a href="{{ asset('storage/' . $item->pdf_file) }}" target="_blank"
                                    class="bg-amber-500 hover:bg-amber-600 text-white px-5 py-2 rounded-xl text-center font-medium transition text-sm">
                                    Lihat PDF
                                </a>
                                @endif

                                <button type="button" @click="approveModal = true; initPad({{ $item->id }});"
                                    class="bg-green-500 hover:bg-green-600 text-white px-5 py-2 rounded-xl font-medium transition text-sm">
                                    Approve
                                </button>

                                <button @click="rejectModal = true" type="button"
                                    class="bg-red-500 hover:bg-red-600 text-white px-5 py-2 rounded-xl font-medium transition text-sm">
                                    Reject
                                </button>
                    ...
                    <div x-show="rejectModal"
                        ...
                        <div @click.away="rejectModal = false" class="bg-white rounded-3xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-xl mx-auto my-8">
                            ...
                            <form method="POST" action="{{ route('hrd.izin.reject', $item->id) }}">
                                @csrf
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-10 text-center">
                    ...
                    <p class="text-gray-500 mt-2">Belum ada pengajuan izin yang menunggu approval HRD</p>
                </div>
